close all; clear;
% Script to solve eKdV with Dirichlet/Neumann BC 
% using 6th order SBP-SAT
%
% u_t + a*u_x + b*u*u_x + c*u_xxx + d*u^2*u_x = 0
...
% coeffs
a = c0; b = 3/2*c0/h0; c = 1/6*c0*h0^2; d = -3/8*c0/h0^2;
ap = a + b*max(u(1),0);
an = a + b*min(u(end),0) + d*u(end)^2;
...
utemp = u;
u = (II - dt/2*A)\( (II + dt/2*A)*u + dt*B*u.^2 + dt*C*u.^3 + dt*E(ap,an)*u + dt*G(2,ap,an) );
Etemp = E(ap,an);
ap = a + b*max(u(1),0) + d*u(end)^2;
an = a + b*min(u(end),0);
for i = 3:N
   utemp2 = u;
   En = E(ap,an);
   u = (II - dt/2*A)\( (II + dt/2*A)*u + dt*(3*(B*u.^2 + C*u.^3 + En*u) - (B*utemp.^2 + C*utemp.^3 + Etemp*utemp))/2 + dt*G(i,ap,an));
   utemp = utemp2;
   Etemp = En; 
    
   ap = b*max(u(1),0) + d*u(end)^2;
   an = b*min(u(end),0);
